    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-white leading-tight uppercase tracking-tighter">
                DISPOSITIVO: <span class="text-cyan-400">{{ $device->name }}</span>
            </h2>
                        ...
                        <div class="space-y-5">
                            @if($device->client)
                                <div>
                                    ...
                                    <p class="text-xl font-bold text-slate-900 leading-none mt-1">{{ $device->client->name }}</p>
                                </div>
                                <div>
                    ...
                    <div class="rounded-lg shadow-inner overflow-hidden">
                        <div class="p-6 {{ $device->status == 'activo' ? 'bg-slate-900' : ($device->status == 'falla' ? 'bg-red-600' : 'bg-orange-500') }} transition-colors duration-500">
                            <div class="flex items-center justify-between">
                                <div>
                                    <p class="text-[10px] {{ $device->status == 'activo' ? 'text-gray-400' : 'text-white/80' }} uppercase font-black tracking-widest">Estado del Sistema</p>
                                    <div class="flex items-center mt-2 gap-2">
                                        ...
                                        <span class="text-2xl font-black uppercase tracking-tighter text-white">{{ $device->status }}</span>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="text-[10px] {{ $device->status == 'activo' ? 'text-gray-400' : 'text-white/80' }} uppercase font-bold tracking-widest">Tipo</p>
                                    <p class="font-bold text-sm mt-1 uppercase text-white">{{ $device->type }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full">
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="3" class="py-16 text-center text-gray-400 text-sm italic">
                                                No se han detectado anomalías en este dispositivo.
                                            </td>
                                        </tr>
                                    @endforelse
                                </tbody>
